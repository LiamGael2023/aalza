...
class tablaMedidor{
    ...
    public function mostrartablaMedidor() {
        $item = null;
        $valor = null;

        $medidor = ControladorMedidor::ctrMostrarMedidor($item, $valor); 
        ...
        foreach ($medidor as $key => $medidores) {
            ...
            $codigo = '<td>'.$medidores["codigo"].'</td>';
            $marca = '<td>'.$medidores["marca"].'</td>';
            $modelo = '<td>'.$medidores["modelo"].'</td>';
            $tiporegistro = '<td>'.$medidores["tiporegistro"].'</td>';
            $botones = '<td>'
                       . '<div class="btn-group btn-group-sm" role="group">'
                       . '<a type="button" class="btn btn-sm btn-warning p-1 btnEditarModeloMedidor" id='.$medidores["id"].' data-bs-toggle="modal" data-bs-placement="top" data-bs-target="#exampleModalEditar"><svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg></a>'
                       . '<a type="button" href="#" class="btn btn-sm btn-secondary p-1" title="Asignar Lotes/Capatación" onclick="redirectToMedidorDetail('.$medidores["id"].')"><svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-list"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M9 6l11 0" /><path d="M9 12l11 0" /><path d="M9 18l11 0" /><path d="M5 6l0 .01" /><path d="M5 12l0 .01" /><path d="M5 18l0 .01" /></svg></a>'
...
$activarMedidor = new tablaMedidor();
$activarMedidor->mostrartablaMedidor();
